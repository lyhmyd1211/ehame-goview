...
import { PublicGroupConfigClass, PublicModalGroupConfigClass } from '@/packages/public/publicConfig'
...
export const useChartEditStore = defineStore({
  ...
  actions: {
    ...
    setTargetSelectChart(selectId?: string | string[], push: boolean = false) {
      // 重复选中
      if (this.targetChart.selectId.find((e: string) => e === selectId)) {
    ...
    // * 创建分组
    setGroup(id?: string | string[], isHistory = true,isModal=false) {
      try {
        ...
        loadingStart()
        const groupClass = isModal?new PublicModalGroupConfigClass():new PublicGroupConfigClass()
