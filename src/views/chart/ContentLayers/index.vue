  ...
  if (chartEditStore.getTargetChart.selectId.length > 1) {
    if (chartEditStore.getTargetChart.selectId.length ===2) {
      if (chartEditStore.getSelectModal === 1) {
        return targetList.filter(i => [MenuEnum.MODAL, MenuEnum.DELETE].includes(i.key as MenuEnum))
      }else if (chartEditStore.getSelectModal >1) {
        return targetList.filter(i => [ MenuEnum.DELETE].includes(i.key as MenuEnum))
      }
    }else if (chartEditStore.getSelectModal>0) {
      return targetList.filter(i => [ MenuEnum.DELETE].includes(i.key as MenuEnum))
    }
    return targetList.filter(i => [MenuEnum.GROUP, MenuEnum.DELETE].includes(i.key as MenuEnum))
  }
